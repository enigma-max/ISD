...
const HomePage = () => {
  ...
  // Get stored coordinates from chosen location
  const getCoordsFromStorage = () => {
    try {
      const raw = localStorage.getItem("active_location_coords");
      if (!raw) return { latitude: undefined, longitude: undefined };
  ...
  const { restaurants, loading, totalPages } = useNearbyRestaurants({
    page,
    pageSize: PAGE_SIZE,
  ...
  useEffect(() => {
    ...
    // Fallback: get GPS if nothing saved
  ...
  // Handle user confirming location
  const handleLocationConfirmed = () => {
    const loc = localStorage.getItem("active_location") || "";
    setActiveLocation(loc);

    const coords = getCoordsFromStorage();
    setUserCoords(coords); // ✅ update coordinates for nearby restaurants
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="px-4 sm:px-6 py-3">
          <div
            ...
            style={{
              background: "linear-gradient(to right, var(--pink-light), var(--pink-mid))",
            }}
          >
            <img
            ...
            <div className="flex-1 flex flex-col justify-center px-3 sm:px-6 gap-2 sm:gap-4">
              ...
              >
                ...
                <span className="hidden sm:inline">
                  Sign up for <span style={{ color: "var(--pink)" }}>free delivery</span> on
                  your first order!
                </span>
              </p>
        ...
        <div className="px-4 sm:px-6 pt-4">
          <h2 className="font-bold text-foreground text-lg sm:text-xl mb-4">
            Nearby Restaurants
          </h2>
          {loading ? (
      ...
      <BottomNavbar />
      {showLocationPopup && <ConfirmLocation onConfirm={handleLocationConfirmed} />}
    </div>
  );
};
